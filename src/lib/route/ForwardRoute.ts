import {IRawNetPacket, ListenerCallback, Logger, Notify, OPCode, Provider, Request, Response, Route, RPCError, RPCErrorCode, RPCHeader, Runtime, Service} from '@sora-soft/framework';
      ...
      switch (packet.opcode) {
        case OPCode.REQUEST: {
          ...
          if (route.hasMethod(request.method)) {
            ...
            // 转发至其他服务
            const [service, method] = request.path.split('/').slice(-2);
        ...
        case OPCode.NOTIFY: {
          ...
          if (route.hasNotify(notify.method)) {
            ...
            // 转发至其他服务
            const [service, method] = notify.path.split('/').slice(-2);
            const provider: Provider<Route> = Pvd[service];
            if (!provider)
              throw new RPCError(RPCErrorCode.ERR_RPC_PROVIDER_NOT_AVAILABLE, `ERR_RPC_PROVIDER_NOT_AVAILABLE, service=${service}`);
